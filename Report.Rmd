...
```{r echo=FALSE}
data.frame(method = "Just the average", RMSE = 1.060503) %>%
  bind_rows(data.frame(method = "Movie Effect Model", RMSE = 0.9435731))
```
...
data.frame(method = "Just the average", RMSE = 1.060503) %>%
  bind_rows(data.frame(method = "Movie Effect Model", RMSE = 0.9435731)) %>%
  bind_rows(data.frame(method = "Movie + User Effect Model", RMSE = 0.8653122))
```
## 3. Genre effect  
...
```{r Initial genres effect, eval=FALSE, echo=FALSE}
comedy_effect <- train_set %>% 
  left_join(movie_effect, by = 'movieId') %>%
  left_join(user_effect, by = 'userId') %>%
  mutate(comedy = ifelse(str_detect(genres, "Comedy"), "1", "0"), pred = mu + b_u + b_i) %>%
...
RMSE(pred, test_set$rating)
```
Upon completing and running the code, it can be observed that the RMSE for the model with the added genre effect is 0.8757116.
...
data.frame(method = "Just the average", RMSE = 1.060503) %>%
  bind_rows(data.frame(method = "Movie Effect Model", RMSE = 0.9435731)) %>%
  bind_rows(data.frame(method = "Movie + User Effect Model", RMSE = 0.8653122)) %>%
  bind_rows(data.frame(method = "Movie + User + Genre Effect Model", RMSE = 0.8757116))
```
...
pred <- test_set %>% 
  ...
  RMSE(pred, test_set$rating)
```
Upon running the above code, it can be observed that the RMSE for the model with the new genre effect is 0.8461327.
...
data.frame(method = "Just the average", RMSE = 1.060503) %>%
  ...
  bind_rows(data.frame(method = "Movie + User Effect Model", RMSE = 0.8653122)) %>%
  bind_rows(data.frame(method = "Movie + User + Genre Effect Model", RMSE = 0.8757116)) %>%
  bind_rows(data.frame(method = "Regularized Movie-User-Genre Effect Model", RMSE = 0.8461327))
```
...
data.frame(method = "Just the average", RMSE = 1.060503) %>%
  ...
  bind_rows(data.frame(method = "Movie + User + Genre Effect Model", RMSE = 0.8757116)) %>%
  bind_rows(data.frame(method = "Regularized Movie-User-Genre Effect Model", RMSE = 0.8461327)) %>%
  bind_rows(data.frame(method = "Movie Year + Regularized Movie-User-Genre Effect Model", RMSE = 0.8459691))
```
